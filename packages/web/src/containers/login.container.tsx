  ...
  return {
    customSubmit: function(client: ApolloClient<any>) {
      return async (values: { email: string; password: string }) => {
        return client
          .query<any, { email: string; password: string }>({
            query: LOGIN,
            variables: {
              email: values.email,
          ...
          .then(value => {
            ...
            let user: IUser = {
              id: login.user._id,
              token: login.token,
              availableForms: login.user.availableForms.map(
                (item: any) => item.id
              ),
              ...
              createdForms: login.user.createdForms
                ? login.user.createdForms.map((item: any) => item.id)
                : []
            };
            let rootForms: any = {};
            ...
            login.user.filledForms.forEach((v: any) => {
              if (typeof v.answerStore === "string")
                v.answerStore = JSON.parse(v.answerStore);
              filledForms[v.id] = {
