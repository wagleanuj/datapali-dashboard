...
class FormsComponent extends React.Component<FormsProps, FormsState>{
    ...
    getAutoCompleteDataForPath(filledForm: FilledForm): (path: number[], iteration: number) => AutoCompleteItem[] {
        ...
        return function (questionId: string) {
            let answers: { [key: string]: AutoCompleteItem } = {};
            Object.keys(this.state.filledForms).forEach(key => {
                const form: FilledForm = this.state.filledForms[key];
                if (formID === form.formId && form.id !== filledFormID) {
                    const answerSection = form.answerSection;
                    if (!answerSection.hasCache()) answerSection.buildCache();
                    const a = answerSection.getCache().getAnswerFor(questionId);
                    a.forEach(item => {
                        if (!answers[item.value]) answers[item.value] = { text: item.value, strength: 1 };
                        else {
                            answers[item.value].strength++;
                        }
                    });
    ...
    async handleAddNewForm(formId?: string) {
        ...
        let newFilledForm: FilledForm = {
            answerSection: new AnswerSection(rootForm).buildCache(),
    ...
    async loadSurveyForm(filledFormId: string) {
        ...
        filledform.answerSection.buildCache();
