...
const NumberRange = (props: NumberRangeProps = defaultNumberRangeProps) => {
    ...
    return (
        <ButtonGroup>
            <input defaultValue={props && props.range.min && props.range.min.toString()}
                onChange={e => {
            ...
            <Divider></Divider>
            <input defaultValue={props && props.range.max && props.range.max.toString()} onChange={e => {
...
const TimeRange = (props: TimeRangeProps = defaultTimeRangeProps) => {
    ...
    return (
        <ButtonGroup>
            <TimePicker precision={TimePrecision.MINUTE} useAmPm={true} defaultValue={new Date(minValue)} onChange={newTime => {
                setMin(newTime.toLocaleDateString());
                if (props.onChange) props.onChange({ min: minValue, max: maxValue })
            }}></TimePicker>
            <Divider />

            <TimePicker precision={TimePrecision.MINUTE} useAmPm={true} defaultValue={new Date(maxValue)} onChange={newTime => {
...
export class ValInput extends React.Component<ValInputProps, ValInputState> {
    ...
    render() {
        ...
        switch (this.props.type.name) {
            case ANSWER_TYPES.NUMBER:
                comp = <input type="number" onChange={e => {
            ...
            case ANSWER_TYPES.BOOLEAN:
                ...
                comp = <Select defaultValue={def} options={opt} onChange={(newVal: any) => {
            ...
            case ANSWER_TYPES.STRING:
                comp = <input type="text" onChange={e => {
